    ...
    <div className="continue-card" style={{ '--cc-color': found.course.color }}
      ...
      <div className="continue-card__right">
        <div className="continue-card__type-badge">
          {found.mat.type === 'video' ? '▶ Video' : '📖 Reading'}
        </div>
    ...
    <div className="material-viewer">
      ...
      <div className="material-viewer__bar">
        ...
        <div className="mv-meta">
          <span className={`badge ${material.type === 'video' ? 'badge--video' : 'badge--text'}`}>
            {material.type === 'video' ? '▶ Video' : '📄 Reading'}
          </span>
...
function AdminCourseEditor({ course, courses, setCourses, saveAll, onBack }) {
  ...
  const [addingMat, setAddingMat] = useState(null);
  const [newMat, setNewMat] = useState({ type: 'text', title: '', videoUrl: '', content: [] });
  ...
  function addWeek() {
    setLocal({ ...local, weeks: [...local.weeks, { id: genId(), order: local.weeks.length, title: `Week ${local.weeks.length + 1}: New Week`, description: '', materials: [] }] });
  }
  function removeWeek(weekId) { setLocal({ ...local, weeks: local.weeks.filter(w => w.id !== weekId) }); }
  function updateWeekTitle(weekId, title) { setLocal({ ...local, weeks: local.weeks.map(w => w.id === weekId ? { ...w, title } : w) }); }
  function addMaterial(weekId) {
    if (!newMat.title.trim()) return;
    const mat = { id: genId(), type: newMat.type, title: newMat.title, duration: '~10 min' + (newMat.type === 'text' ? ' read' : ''), ...(newMat.type === 'video' ? { videoUrl: newMat.videoUrl, description: newMat.title } : { content: newMat.content }) };
    setLocal({ ...local, weeks: local.weeks.map(w => w.id === weekId ? { ...w, materials: [...w.materials, mat] } : w) });
    setNewMat({ type: 'text', title: '', videoUrl: '', content: [] }); setAddingMat(null);
  }
  function removeMaterial(weekId, matId) { setLocal({ ...local, weeks: local.weeks.map(w => w.id === weekId ? { ...w, materials: w.materials.filter(m => m.id !== matId) } : w) }); }
  function addContentBlock() {
    if (!newContentText.trim()) return;
    setNewMat({ ...newMat, content: [...newMat.content, { type: newContentType, text: newContentText }] });
    setNewContentText('');
  }
    ...
    <div className="admin-editor">
      ...
      <div className="editor-meta">
        <div className="editor-field">
          ...
          <input className="editor-input" value={local.title} onChange={e => setLocal({ ...local, title: e.target.value })} />
        </div>
        <div className="editor-field">
          <label>Description</label>
          <textarea className="editor-input editor-textarea" value={local.description} onChange={e => setLocal({ ...local, description: e.target.value })} />
        </div>
      </div>

      <h3 className="editor-section-title">Weeks & Materials</h3>
      {local.weeks.map((week, idx) => (
        <div key={week.id} className="editor-week">
          <div className="editor-week__head">
            <input className="editor-input editor-week__title-input" value={week.title} onChange={e => updateWeekTitle(week.id, e.target.value)} />
          ...
          <div className="editor-week__materials">
            {week.materials.map(mat => (
              <div key={mat.id} className="editor-mat">
                <span className="editor-mat__type">{mat.type === 'video' ? '▶' : '📄'}</span>
          ...
          {addingMat === week.id ? (
            <div className="editor-add-mat">
              <div className="editor-add-mat__row">
                <select className="editor-input" value={newMat.type} onChange={e => setNewMat({ ...newMat, type: e.target.value })}>
                  <option value="text">Text / Reading</option>
                  <option value="video">Video</option>
                </select>
                <input className="editor-input" placeholder="Material title" value={newMat.title} onChange={e => setNewMat({ ...newMat, title: e.target.value })} />
              </div>
              {newMat.type === 'video' && (
                <input className="editor-input" placeholder="YouTube URL" value={newMat.videoUrl} onChange={e => setNewMat({ ...newMat, videoUrl: e.target.value })} />
              )}
              {newMat.type === 'text' && (
                <div className="editor-content-builder">
                  <div className="editor-content-builder__row">
                    <select className="editor-input" value={newContentType} onChange={e => setNewContentType(e.target.value)}>
                      <option value="paragraph">Paragraph</option>
                      <option value="header">Header</option>
                      <option value="highlight">Highlight</option>
                      <option value="quote">Quote</option>
                    </select>
                    <input className="editor-input" placeholder="Text content..." value={newContentText} onChange={e => setNewContentText(e.target.value)} />
                    <button className="tbl-btn tbl-btn--approve" onClick={addContentBlock}>+ Add Block</button>
                  </div>
                  {newMat.content.length > 0 && (
                    <div className="content-preview">
                      {newMat.content.map((b, i) => <div key={i} className="content-preview__item"><span className="content-preview__type">{b.type}</span><span>{b.text}</span></div>)}
                    </div>
                  )}
                </div>
              )}
              <div className="editor-add-mat__actions">
                ...
                <button className="btn btn--ghost" onClick={() => setAddingMat(null)}>Cancel</button>
              </div>
            </div>
          ) : (
            <button className="editor-add-btn" onClick={() => setAddingMat(week.id)}>+ Add Material</button>
          )}
        </div>
      ))}
