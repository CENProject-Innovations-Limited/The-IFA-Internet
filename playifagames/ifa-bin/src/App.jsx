...
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.err) {
      return (
        <div style={{padding:'2rem',background:'#0b0d15',color:'#ff6b6b',fontFamily:'monospace',minHeight:'100vh'}}>
          <h2 style={{color:'#ff6b6b'}}>Something went wrong</h2>
          <pre style={{whiteSpace:'pre-wrap',fontSize:'0.8rem',opacity:0.8}}>
            {this.state.err.message}{'\n'}{this.state.err.stack}
          </pre>
          <button style={{marginTop:'1rem',padding:'0.5rem 1rem',background:'#c9a227',border:'none',borderRadius:'6px',cursor:'pointer'}}
...
function BinGame({ onEnd, startSubMode = null }) {
  ...
  const S = useRef({
    ...
    subMode: startSubMode || null, roundType: 'd2b', playerDecimal: '',
  ...
  const [subMode,       setSubMode]       = useState(startSubMode || null);  // null = show picker
  ...
  function doSubmit() {
    ...
    revealRef.current = setTimeout(() => {
      if (over) {
        onEnd({ score: sc, subMode: S.current.subMode });
      } else {
        ...
        <div className="bin__pick-section bin__pick-section--anc">
          <div className="bin__pick-heading bin__pick-heading--anc">
            ...
            <span className="bin__pick-heading-text">
              ...
              <span className="bin__pick-heading-sub">Yoruba · Isese · Odù Ifá · Odu Oosa</span>
            </span>
          </div>
          <div className="bin__submode-pick bin__submode-pick--anc">
            <button className="bin__submode-card bin__submode-card--opele" onClick={() => chooseSubMode('opele')}>
              ...
              <div className="bin__submode-name">Ọ̀pẹ̀lẹ̀ Ifá</div>
              <div className="bin__submode-desc">
                8 Opele seeds · Ogbe (O) = open · Oyeku (|) = closed<br/>The original 8-bit binary system
              </div>
              <div className="bin__submode-tag bin__submode-tag--gold">Odù Ifá · 8-bit</div>
            </button>
            <button className="bin__submode-card bin__submode-card--obi" onClick={() => chooseSubMode('obi')}>
              ...
              <div className="bin__submode-name">Obì Sísọ</div>
              <div className="bin__submode-desc">
            ...
            <button className="bin__submode-card bin__submode-card--cowrie" onClick={() => chooseSubMode('cowrie')}>
              ...
              <div className="bin__submode-name">Ẹ́rìndínlógún</div>
              <div className="bin__submode-desc">
                16 Ajé cowries · mouth-up = open · back-up = silent<br/>Ancestor of hexadecimal
              </div>
            ...
            <button className="bin__submode-card bin__submode-card--odu4" onClick={() => chooseSubMode('odu4')}>
              ...
              <div className="bin__submode-name">Odù Ifá Codes</div>
              <div className="bin__submode-desc">
                Read the 4-bit Odu mark pattern · Identify the Odu<br/>Ikin · Opele · Agbigba
              </div>
              <div className="bin__submode-tag bin__submode-tag--gold">Odù Ifá · 4-bit MCQ</div>
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="bin">
      ...
      {/* Banner */}
      <div className="bin__banner">
        <button className="bin__back-btn" onClick={goBack}>← Games</button>
        <div className="bin__logo-chip">
      ...
      {roundType === 'opele' && (
        <>
          <div className={`bin__card anc-card${isReveal ? (isCorrect ? ' bin__card--ok' : ' bin__card--no') : ''}`}>
            <div className="bin__card-label anc-card__lbl">Round {round + 1} / {BIN_ROUNDS} · Ọ̀pẹ̀lẹ̀ Ifá — 8-bit encoding</div>
          ...
          <div className="opele-chain-wrap">
            <div className="opele-chain-header">
              ...
              <span className="opele-chain-header__label">Ọ̀pẹ̀lẹ̀ Ifá · 8-bit Binary Chain</span>
              <span className="opele-chain-header__line"/>
            </div>
            ...
            <div className="opele-odu-row">
              <span className="opele-odu-row__label">Odù Ifá →</span>
      ...
      {roundType === 'obi' && (
        <>
          <div className={`bin__card anc-card${isReveal ? (isCorrect ? ' bin__card--ok' : ' bin__card--no') : ''}`}>
            <div className="bin__card-label anc-card__lbl">Round {round + 1} / {BIN_ROUNDS} · Obì Sísọ — 4-bit Kola Nut</div>
            <div className="anc-card__intro">
          ...
          <div className="obi-wrap">
            <div className="obi-header">
              ...
              <span className="obi-header__label">Obì Sísọ · 4-bit Divination Cast</span>
              <span className="obi-header__line"/>
            </div>
        ...
        <>
          ...
          <div className="cowrie-wrap">
            <div className="cowrie-header">
              ...
              <span className="cowrie-header__label">Ẹ́rìndínlógún · 16 Ajé Cowries</span>
              <span className="cowrie-header__line"/>
            </div>
      ...
      {roundType === 'odu4' && odu4Q && (
        <>
          <div className={`bin__card anc-card${isReveal ? (isCorrect ? ' bin__card--ok' : ' bin__card--no') : ''}`}>
            <div className="bin__card-label anc-card__lbl">Round {round + 1} / {BIN_ROUNDS} · Odù Ifá — 4-bit Sacred Code</div>
            <div className="anc-card__intro">
...
function App() {
  ...
  // ── DIAGNOSTIC: capture async/event-handler errors (not caught by ErrorBoundary) ──
  const [dbgErrs, setDbgErrs] = useState([]);
  useEffect(() => {
    const onErr = (msg, src, line, col, err) => {
      setDbgErrs(prev => [...prev.slice(-3), `ERR: ${msg} @${line}:${col}`]);
    };
    const onReject = (e) => {
      setDbgErrs(prev => [...prev.slice(-3), `REJECT: ${e.reason}`]);
    };
  ...
  const [lastBinSubMode, setLastBinSubMode] = useState(null);
  ...
  function handleBinEnd(res) {
    ...
    if (res.subMode) setLastBinSubMode(res.subMode);
    setResult(res);
    setScreen('result');
  }
  ...
  return (
    <div className="app">
      ...
      <main className="main">
        ...
        {screen === 'bin' && (
          <BinGame key={gameKey} onEnd={handleBinEnd} startSubMode={lastBinSubMode} />
        )}
      ...
      {/* ── DIAGNOSTIC PANEL (remove after fix) ── */}
      <div style={{
        ...
        <span style={{color:'#ff0'}}>screen:{screen}</span>
        {' | mode:'}{mode}
        {' | key:'}{gameKey}
        {' | sub:'}{lastBinSubMode || '∅'}
        {' | odus:'}{odus.length}
        {dbgErrs.map((e,i) => (
          <div key={i} style={{color:'#f66'}}>{e}</div>
        ))}
      </div>
    </div>
  );
