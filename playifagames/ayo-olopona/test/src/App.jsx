  ...
  return (
    <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" style={{ display:'block', width:'100%', height:'100%' }}>
      ...
      {figAngles.map((angle, i) => {
        ...
        if (isEshu) {
          // Eshu face — sacred guardian at crown of every Opon Ifa
          return (
        ...
        // Regular carved Orisa/ancestor figure
        return (
      ...
      {/* Odu Marks — Meji form: left column + right column (identical, doubled) */}
      {/* Rows: cy-24, cy-8, cy+8, cy+24 — mark height ±7 — 2px gap between rows */}
      <g filter={`url(#${uid}-mg)`}>
        {[cx - 18, cx + 18].map((colX, ci) =>
          marks.map((bit, ri) => {
            const baseY = cy - 24 + ri * 16;
            if (bit === '1') {
              // Single mark — one vertical line
              return (
                <line key={`${ci}-${ri}`}
                  x1={colX} y1={baseY - 7}
                  x2={colX} y2={baseY + 7}
            ...
            // Double mark — two vertical lines
            return (
              <g key={`${ci}-${ri}`}>
      ...
      {/* Meji centre divider (subtle dashed) */}
...
function OponIfaMiniCircle({ odu, dualOdu, onClick }) {
  ...
  function handleClick() {
    if (busy.current) return;
    busy.current = true;
    setVisible(false);          // trigger fade-out + shrink CSS
    setTimeout(() => {
      setShowingDual(d => !d);  // swap Odu while invisible
      setVisible(true);         // trigger fade-in + grow CSS
      onClick();
        ...
        <div className="challenge-art-panel">
          ...
          <div className="opon-ifa-board">
            <div className="opon-ifa-row">
              {P1_DISP.map(i => (
                <OponIfaMiniCircle key={i} odu={ODU[i]} dualOdu={ODU[dualIdx(i)]}
...
// ── Diagnostic error boundary — shows crash message on screen ──
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.err) {
      return (
        <div style={{
          position:'fixed', inset:0, background:'#0a0614', color:'#ff6b6b',
          fontFamily:'monospace', padding:'40px 32px', overflowY:'auto', zIndex:99999,
        }}>
          <div style={{ fontSize:'13px', fontWeight:700, marginBottom:'12px', color:'#f0c840' }}>
            ⚠ RENDER ERROR — Check browser console (F12) for full trace
          </div>
          <pre style={{ fontSize:'11px', lineHeight:1.6, whiteSpace:'pre-wrap', color:'#e87878' }}>
